...
{
Employee.findOne({name:nameSearch},function (err,doc){
console.log(doc);
});
}
...
{
Employee.findOne({e_id:idSearch},function (err,doc){
console.log(doc);
});
}
...
{
	Employee.findOne({manager_id:idSearch},function (err,doc){
...
{
Employee.findOneAndRemove({ e_id:id }, function(err) {
  if (err) throw err;
console.log('employee deleted!');
});
Employee.update({ e_id: id}, { $set: { manager_id: null }}, function(err){
if(err) throw err;
console.log('updated manager');
});
Project.update({ managed_by: id }, { $set: { managed_by: null }}, function(err){
if(err) throw err;
console.log('managed by updated');
});
Project.update({},{$pull: {employees: id}}, {multi: true},function(err)
{
...
{
Employee.update({ e_id: id}, { $set: { name: nameNew }}, function(err){
if(err) throw err;
console.log('updated');
});
...
{
Employee.update({ e_id: id}, { $set: { manager_id: managerId }}, function(err){
if(err) throw err;
console.log('updated manager');
});
}